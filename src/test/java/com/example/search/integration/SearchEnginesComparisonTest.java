...
import com.example.search.dto.DocumentInputDto;
import com.example.search.dto.SearchComparisonDto;
 ...
 * 
 * 1. APACHE SOLR - полнотекстовый поиск корпоративного уровня
 *    - Faceted search (фасетный поиск)
 *    - Highlighting (подсветка совпадений)
 *    - Analytics и aggregations
 *    - Масштабируемость через SolrCloud
 *    - Богатые query parsers (edismax, standard)
 * 
 * 2. OPENSEARCH - форк Elasticsearch для распределенного поиска
 *    - Распределенная архитектура из коробки
 *    - Мощные aggregations
 *    - Полнотекстовый и структурированный поиск
 *    - Dashboards для визуализации
 *    - Совместимость с Elasticsearch клиентами
 * 
 * 3. TYPESENSE - быстрый typo-tolerant поиск
 *    - Поиск за миллисекунды (in-memory индексы)
 ...
 * - Solr: Enterprise-поиск, сложная аналитика, legacy системы
 * - OpenSearch: Логи, метрики, большие объемы данных, Kibana-like визуализация
 * - TypeSense: E-commerce, автокомплит, поиск по каталогам, быстрый поиск
 * 
 * ТРЕБУЕТ: docker-compose up -d для реальных тестов поиска
 */
@SpringBootTest
@ActiveProfiles("test")
...
public class SearchEnginesComparisonTest {
     ...
     */
    @BeforeAll
    static void setupTestData(@Autowired DocumentService documentService, 
        ...
        input1.setContent("Apache Solr - это open-source платформа для поиска, построенная на Apache Lucene. " +
        ...
        input2.setContent("OpenSearch - это форк Elasticsearch, поддерживаемый AWS. " +
        ...
        input3.setContent("TypeSense - это современная альтернатива Algolia. " +
    ...
    /**
     * ТЕСТ 1: Поиск с Apache Solr
     * Демонстрирует Solr-специфичные возможности
     */
    @Test
    @Order(1)
    ...
    void testSolrSearch() {
        ...
        results.forEach(result -> {
            System.out.println(String.format("  📄 [Score: %.2f] %s", result.getScore(), result.getTitle()));
        });
    ...
    /**
     * ТЕСТ 2: Поиск с OpenSearch
     * Демонстрирует distributed search capabilities
     */
    @Test
    @Order(2)
    ...
    /**
     * ТЕСТ 3: Поиск с TypeSense
     * Демонстрирует typo tolerance и скорость
     */
    @Test
    @Order(3)
    ...
    /**
     * ТЕСТ 4: Сравнение производительности всех движков
     * Главная фича проекта!
     */
    @Test
    @Order(4)
    ...
    void testSearchEnginesPerformanceComparison() {
        ...
        int minTime = Math.min(comparison.getSolrTime(), 
                      Math.min(comparison.getOpenSearchTime(), comparison.getTypeSenseTime()));
        String fastest = minTime == comparison.getSolrTime() ? "Solr" :
    ...
    /**
     * ТЕСТ 5: Демонстрация различий в ранжировании
     * Показывает как разные движки оценивают релевантность
     */
    @Test
    @Order(5)
    ...
    void testRankingDifferences() {
        ...
        solrResults.stream().limit(3).forEach(r ->
            System.out.println(String.format("   %.3f - %s", r.getScore(), r.getTitle()))
        );
        
        System.out.println("\nOPENSEARCH RANKING (BM25 with tweaks):");
        osResults.stream().limit(3).forEach(r ->
            System.out.println(String.format("   %.3f - %s", r.getScore(), r.getTitle()))
        );
        
        System.out.println("\nTYPESENSE RANKING (Typo-tolerance + relevance):");
        tsResults.stream().limit(3).forEach(r ->
            System.out.println(String.format("   %.3f - %s", r.getScore(), r.getTitle()))
        );
    ...
    static void cleanup() {
        ...
        System.out.println("   3. Откройте http://localhost:8080/graphiql для интерактивных запросов\n");
    }
}
