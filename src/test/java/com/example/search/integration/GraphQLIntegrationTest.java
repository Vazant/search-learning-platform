...
/**
 * Интеграционный тест для GraphQL API Демонстрирует основные операции: создание, поиск, сравнение
 * движков
 *
 * <p>ВАЖНО: Для работы тестов с реальным поиском нужно запустить: docker-compose up -d
 *
 * <p>Этот тест показывает: 1. Как создавать документы через GraphQL mutations 2. Как искать через
 * разные движки (Solr, OpenSearch, TypeSense) 3. Как сравнивать производительность поисковых
 * движков 4. Как индексировать данные
 */
@SpringBootTest
...
public class GraphQLIntegrationTest {
  ...
  /** ПРИМЕР 1: Создание документа Показывает базовую GraphQL mutation для создания документа */
  @Test
  @Order(1)
  void whenCreateDocument_thenReturnsCreatedDocument() {
    ...
    graphQlTester
        ...
        .path("createDocument.title")
        .entity(String.class)
        .isEqualTo("Введение в Apache Solr")
        .path("createDocument.author")
        .entity(String.class)
        .isEqualTo("Иван Иванов");
  }

  /** ПРИМЕР 2: Получение всех документов Показывает простой GraphQL query */
  @Test
  @Order(2)
  void whenQueryAllDocuments_thenReturnsDocumentList() {
    // Сначала создаем несколько документов
    createSampleDocument(
        "GraphQL Best Practices", "Руководство по эффективному использованию GraphQL");
  ...
  /** ПРИМЕР 3: Получение конкретного документа по ID Показывает GraphQL query с параметрами */
  @Test
  @Order(3)
  void whenQueryDocumentById_thenReturnsDocument() {
    ...
    graphQlTester
        ...
        .path("document.title")
        .entity(String.class)
        .isEqualTo("TypeSense Speed Benchmark");
  }

  /** ПРИМЕР 4: Обновление документа Показывает GraphQL mutation с ID и input */
  @Test
  @Order(4)
  void whenUpdateDocument_thenReturnsUpdatedDocument() {
    ...
    graphQlTester
        ...
        .entity(String.class)
        .isEqualTo(docId)
        .path("updateDocument.title")
        .entity(String.class)
        .isEqualTo("Updated Title")
        .path("updateDocument.content")
        .entity(String.class)
        .satisfies(
            content -> {
  ...
  /**
   * ПРИМЕР 5: Индексация документа во все поисковые движки Показывает как работает multi-engine
   * indexing
   *
   * <p>ТРЕБУЕТ: запущенный docker-compose up -d
   */
  @Test
  @Order(5)
  void whenIndexDocument_thenIndexedInAllEngines() {
    ...
    graphQlTester
        ...
        .variable("id", docId)
        .execute()
        .path("indexDocument.documentId")
        .entity(String.class)
        .isEqualTo(docId)
        .path("indexDocument.message")
        .entity(String.class)
        .satisfies(
            msg -> {
  ...
  /**
   * ПРИМЕР 6: Сравнение всех поисковых движков Самый интересный пример - показывает разницу в
   * производительности
   *
   * <p>ТРЕБУЕТ: docker-compose up -d + проиндексированные данные
   */
  @Test
  @Order(6)
  void whenCompareSearchEngines_thenReturnsPerformanceMetrics() {
    ...
    String doc1 =
        createSampleDocument("Elasticsearch vs Solr", "Сравнение возможностей поисковых платформ");
    ...
    graphQlTester
        ...
        .execute()
        .path("compareSearchEngines.query")
        .entity(String.class)
        .isEqualTo("search")
        .path("compareSearchEngines.solrTime")
        .entity(Integer.class)
        .satisfies(
            time -> {
              System.out.println("⏱ Solr search time: " + time + "ms");
            })
        .path("compareSearchEngines.openSearchTime")
        .entity(Integer.class)
        .satisfies(
            time -> {
              System.out.println("⏱ OpenSearch search time: " + time + "ms");
            })
        .path("compareSearchEngines.typeSenseTime")
        .entity(Integer.class)
        .satisfies(
            time -> {
  ...
  /** ПРИМЕР 8: Реиндексация всех документов Показывает batch операцию */
  @Test
  @Order(8)
  void whenReindexAll_thenReturnsStatistics() {
    ...
    graphQlTester
        .document(mutation)
        .execute()
        .path("reindexAll.totalDocuments")
        .entity(Integer.class)
        .satisfies(
            total -> {
