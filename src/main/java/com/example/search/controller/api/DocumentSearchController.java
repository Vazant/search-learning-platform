 ...
 *
 * <p>Endpoints: - POST /documents/search - поиск с фильтрами (JSON body) - POST /documents/facets -
 * получение facets (JSON body) - POST /documents/autocomplete - автокомплит (JSON body)
 */
@RestController
...
@Tag(
    ...
    description = "API для поиска документов с фильтрами, сортировкой, пагинацией и facets")
public class DocumentSearchController {
  ...
  public ResponseEntity<DocumentSearchResponse> searchDocuments(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description =
                  "JSON body с параметрами поиска. Все параметры опциональны. Можно комбинировать любые фильтры.",
              required = false,
              content =
                  @Content(
                      ...
                      examples = {
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Поиск по Java",
                            value =
                                "{\"query\":\"Java\",\"page\":0,\"size\":20,\"sortBy\":\"createdAt\",\"sortOrder\":\"DESC\"}",
                            description =
                                "Найдет: 'Java 17 LTS: Новые возможности', 'Spring Boot 3: Миграция и новые возможности'"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Одобренные документы",
                            value =
                                "{\"category\":\"document\",\"status\":\"approved\",\"page\":0,\"size\":20,\"sortBy\":\"createdAt\",\"sortOrder\":\"DESC\"}",
                            description = "14 одобренных документов категории 'document'"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Поиск по автору",
                            value =
                                "{\"author\":\"Алексей\",\"page\":0,\"size\":20,\"sortBy\":\"createdAt\",\"sortOrder\":\"DESC\"}",
                            description =
                                "Найдет 2 документа: 'Apache Solr' и 'Spring Boot 3' (автор: Алексей Иванов)"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Поиск по поисковым движкам",
                            value =
                                "{\"query\":\"поиск\",\"category\":\"document\",\"status\":\"approved\",\"page\":0,\"size\":20,\"sortBy\":\"createdAt\",\"sortOrder\":\"DESC\"}",
                            description =
                                "Найдет: 'Apache Solr', 'OpenSearch vs Elasticsearch', 'TypeSense', 'E-commerce поиск', 'Log Analysis с OpenSearch'"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Все документы",
                            value =
                                "{\"page\":0,\"size\":20,\"sortBy\":\"createdAt\",\"sortOrder\":\"DESC\"}",
                            description =
                                "Все 24 документа, отсортированные по дате создания (новые первыми)")
                      }))
          @RequestBody
          @Valid
    ...
    log.info(
        "Request params: query='{}', category='{}', status='{}', author='{}', page={}, size={}",
        request.getQuery(),
        request.getCategory(),
        request.getStatus(),
        request.getAuthor(),
        ...
        request.getSize());
    log.info(
        "Date params: createdAfter={}, createdBefore={}, updatedAfter={}, updatedBefore={}",
                  ...
                  @Content(
                      ...
                      examples = {
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            ...
                            description =
                                "Получить facets для всех измерений (category, status, author)"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Facets для одобренных документов",
                            value =
                                "{\"status\":\"approved\",\"dimensions\":[\"category\",\"author\"]}",
                            description =
                                "Получить facets по категориям и авторам для одобренных документов"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Facets для документов про Java",
                            value = "{\"query\":\"Java\",\"dimensions\":[\"category\",\"status\"]}",
                            description =
                                "Получить facets по категориям и статусам для документов про Java"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            name = "Только категории",
                            value = "{\"dimensions\":[\"category\"]}",
                            description =
                                "Получить только facets по категориям: document (20), task (3), approval (1), attachment (1)")
                      }))
          @RequestBody
          @Valid
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "400",
            description =
                "Неверные параметры (prefix < 2 символов, неверное поле, limit вне диапазона)")
      })
                  ...
                  @Content(
                      ...
                      examples = {
                        ...
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            ...
                            description =
                                "Вернет: 'Apache Solr: Введение в полнотекстовый поиск', 'E-commerce поиск: Требования и решения'"),
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                        ...
                        @io.swagger.v3.oas.annotations.media.ExampleObject(
                            ...
                            description =
                                "Вернет: 'Spring Boot 3: Миграция и новые возможности', 'Security в Spring Boot приложениях'")
                      }))
          @RequestBody
          @Valid
