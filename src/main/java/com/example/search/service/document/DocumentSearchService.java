...
/**
 * Сервис для поиска документов через REST API
 * Использует Criteria API через кастомный репозиторий
 */
@Service
...
public class DocumentSearchService {
    ...
    public DocumentSearchResponse searchDocuments(@Valid DocumentSearchRequest request) {
        log.info("Search request: query={}, category={}, status={}, author={}, page={}, size={}",
                request.getQuery(), request.getCategory(), request.getStatus(), request.getAuthor(),
                request.getPage(), request.getSize());
        log.info("Date filters: createdAfter={}, createdBefore={}, updatedAfter={}, updatedBefore={}",
        ...
        // Конвертация в DTO
        List<DocumentDto> content = resultPage.getContent().stream()
                .map(this::toDto)
    ...
    /**
     * Получить facets по запрошенным измерениям
     * GET /documents/facets
     */
    public Map<String, List<FacetDto>> getFacets(@Valid FacetRequest request) {
        log.info("Facets request: query={}, dimensions={}", request.getQuery(), request.getDimensions());
        ...
        for (String dimension : request.getDimensions()) {
            ...
            List<Object[]> facetData = switch (dimension.toLowerCase()) {
            ...
            List<FacetDto> facetDtos = facetData.stream()
    ...
    /**
     * Автокомплит по полю и префиксу
    ...
    public List<AutocompleteResultDto> autocomplete(@Valid AutocompleteRequest request) {
        ...
        log.info("Autocomplete request: field={}, prefix={}, limit={}",
                request.getField(), request.getPrefix(), limit);

        String normalizedPrefix = request.getPrefix().toLowerCase().trim();
        List<String> results = switch (request.getField().toLowerCase()) {
            case DocumentConstants.AUTOCOMPLETE_TYPE_TITLE -> documentRepository.autocompleteTitles(normalizedPrefix, limit);
            case DocumentConstants.AUTOCOMPLETE_TYPE_AUTHOR -> documentRepository.autocompleteAuthors(normalizedPrefix, limit);
            case DocumentConstants.AUTOCOMPLETE_TYPE_CATEGORY -> documentRepository.autocompleteCategories(normalizedPrefix, limit);
            default -> throw new IllegalArgumentException("Unknown field: " + request.getField());
        };

        return results.stream()
                .map(text -> AutocompleteResultDto.builder()
                        .text(text)
