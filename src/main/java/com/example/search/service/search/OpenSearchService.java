 ...
 * 
 * Особенности:
 * - Batch indexing через Bulk API для эффективной индексации
 * - Полная поддержка всех полей модели Document
 * - Правильная обработка ошибок с разделением типов исключений
 * - Поддержка highlight в поисковых результатах
 * 
 * OpenSearch Java Client API (2.x)
 * 
 * References:
...
public class OpenSearchService {
        ...
        try {
            ...
            Query query = Query.of(q -> q
                .multiMatch(MultiMatchQuery.of(mm -> mm
                    .query(queryText != null && !queryText.isEmpty() ? queryText : "*")
            ...
            SearchResponse<Map<String, Object>> response = openSearchClient.search(searchRequest, 
    ...
    public boolean indexDocument(Document document) {
        ...
        try {
            ...
            log.warn("IO error indexing document {} in OpenSearch: {}", 
                    document.getId(), e.getMessage());
            return false;
        } catch (Exception e) {
            log.warn("Error indexing document {} in OpenSearch: {}", 
                    document.getId(), e.getMessage());
            return false;
        }
    }
                ...
                List<BulkOperation> bulkOperations = batch.stream()
                        .map(id -> {
                            org.opensearch.client.opensearch.core.bulk.DeleteOperation deleteOp = 
                                    org.opensearch.client.opensearch.core.bulk.DeleteOperation.of(del -> del
                                            .index(index)
                                            .id(String.valueOf(id))
                                    );
