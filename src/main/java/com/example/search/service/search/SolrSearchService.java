 ...
 * 
 * Особенности:
 * - Batch indexing для эффективной индексации
 * - Полная поддержка всех полей модели Document
 * - Правильная обработка ошибок с retry logic
 * - Поддержка дат и других типов данных
 */
@Service
...
public class SolrSearchService {
    ...
    public List<SearchResultDto> search(String query) {
        ...
        try {
            ...
            solrQuery.setFields("id", "title", "content", "author", "category", "status", "created_at", "updated_at");
            ...
            for (SolrDocument doc : response.getResults()) {
                SearchResultDto result = SearchResultDto.builder()
                        ...
                        .score(doc.getFieldValue("score") != null ? 
     ...
     * Для production рекомендуется использовать batchIndexDocuments
     */
    public boolean indexDocument(Document document) {
        ...
        try {
            ...
            log.debug("Document {} indexed in Solr (status: {})", 
                    document.getId(), response.getStatus());
            return true;
        } catch (SolrServerException e) {
            log.warn("Solr server error indexing document {}: {}", 
                    document.getId(), e.getMessage());
            return false;
        } catch (IOException e) {
            log.warn("IO error indexing document {}: {}", 
                    document.getId(), e.getMessage());
            return false;
        } catch (Exception e) {
            log.error("Unexpected error indexing document {}: {}", 
    ...
    public int batchIndexDocuments(Collection<Document> documents) {
        ...
        try {
            ...
            for (int i = 0; i < solrDocs.size(); i += batchSize) {
                ...
                indexed += batch.size();
                log.debug("Indexed batch of {} documents (status: {})", 
                        batch.size(), response.getStatus());
            }
    ...
    public int batchDeleteDocuments(Collection<Long> ids) {
        ...
        try {
            ...
            log.info("Deleted {} documents from Solr (status: {})", 
                    ids.size(), response.getStatus());
            return ids.size();
        } catch (SolrServerException e) {
