...
/**
 * Простой поисковый сервис с фильтрами, сортировкой и пагинацией
 * Использует JPA/Hibernate для поиска в базе данных
 */
@Service
...
public class SimpleSearchService {
    ...
    public SearchResultPageDto searchDocuments(SearchFilterDto filter, String userId) {
        log.info("Searching documents with filters: query={}, category={}, status={}, author={}, page={}, size={}",
                filter.getQuery(), filter.getCategory(), filter.getStatus(), filter.getAuthor(),
        ...
        if (userId != null) {
            List<Long> documentIds = filteredContent.stream()
                    .map(Document::getId)
        ...
        if (Boolean.TRUE.equals(filter.getIncludeFacets())) {
            facets = buildFacets(filter.getQuery(), userId);
        }

        // Пересчитываем пагинацию после фильтрации по правам
        long totalElements = userId != null 
                ? filteredContent.size() // Упрощенный подсчет, в production нужен полный запрос
     ...
     */
    private SearchFacetsDto buildFacets(String query, String userId) {
        ...
        List<FacetDto> categoryFacets = documentRepository.getCategoryFacets(params).stream()
                .map(row -> FacetDto.builder()
                        .value((String) row[0])
        ...
        List<FacetDto> statusFacets = documentRepository.getStatusFacets(params).stream()
                .map(row -> FacetDto.builder()
                        .value((String) row[0])
                        .count(((Number) row[1]).longValue())
                        .label((String) row[0])
        ...
        List<FacetDto> authorFacets = documentRepository.getAuthorFacets(params).stream()
                .map(row -> FacetDto.builder()
                        .value((String) row[0])
                        .count(((Number) row[1]).longValue())
                        .label((String) row[0])
     ...
     */
    public List<AutocompleteResultDto> autocompleteTitles(String prefix, Integer limit, String userId) {
        log.debug("Autocomplete titles with prefix: {} for user: {}", prefix, userId);
        int size = limit != null && limit > 0 ? limit : 10;
        
        List<String> titles = documentRepository.autocompleteTitles(
                prefix != null ? prefix.toLowerCase() : "", 
                size
        );
        ...
        return titles.stream()
                .map(title -> AutocompleteResultDto.builder()
                        .text(title)
     ...
     */
    public List<AutocompleteResultDto> autocompleteAuthors(String prefix, Integer limit, String userId) {
        log.debug("Autocomplete authors with prefix: {} for user: {}", prefix, userId);
        int size = limit != null && limit > 0 ? limit : 10;
        
        List<String> authors = documentRepository.autocompleteAuthors(
                prefix != null ? prefix.toLowerCase() : "", 
                size
        );
        ...
        return authors.stream()
                .map(author -> AutocompleteResultDto.builder()
                        .text(author)
     ...
     */
    public List<AutocompleteResultDto> autocompleteCategories(String prefix, Integer limit, String userId) {
        log.debug("Autocomplete categories with prefix: {} for user: {}", prefix, userId);
        int size = limit != null && limit > 0 ? limit : 10;
        
        List<String> categories = documentRepository.autocompleteCategories(
                prefix != null ? prefix.toLowerCase() : "", 
                size
        );
        ...
        return categories.stream()
                .map(category -> AutocompleteResultDto.builder()
                        .text(category)
    ...
    public List<AutocompleteResultDto> autocomplete(String prefix, String type, Integer limit, String userId) {
        ...
        if (type == null || type.isEmpty() || "ALL".equalsIgnoreCase(type)) {
            ...
            List<AutocompleteResultDto> titleResults = autocompleteTitles(normalizedPrefix, resultLimit * 2, userId);
            List<AutocompleteResultDto> authorResults = autocompleteAuthors(normalizedPrefix, resultLimit, userId);
            List<AutocompleteResultDto> categoryResults = autocompleteCategories(normalizedPrefix, resultLimit, userId);
    ...
    /**
     * Сортировка результатов автокомплита по релевантности
     * Точные совпадения в начале строки имеют приоритет
     */
    private List<AutocompleteResultDto> sortByRelevance(List<AutocompleteResultDto> results, String prefix) {
        return results.stream()
                .sorted((a, b) -> {
                    ...
                    // Если оба начинаются или не начинаются, сортируем по длине (короче = релевантнее)
