...
/**
 * Specification для фильтрации документов с использованием Criteria API Оптимизированная реализация
 * с использованием лучших практик Spring Data JPA
 *
 * <p>Улучшения: - Использование констант для имен полей (избегаем опечаток) - Оптимизированное
 * комбинирование спецификаций (Stream API) - Эффективная обработка null значений - Минимизация
...
public class DocumentSpecification {
   ...
   *
   * <p>⚠️ ПРОИЗВОДИТЕЛЬНОСТЬ: - `LOWER(field) LIKE '%query%'` НЕ использует индексы (full table
   * scan) - На больших таблицах (>10K записей) это будет МЕДЛЕННО - Рекомендуется использовать
   * внешние поисковые движки (Solr, OpenSearch, TypeSense) для полнотекстового поиска в production
   *
   * <p>Оптимизации: - Порядок полей: title (короче) → author → content (длиннее) - Использует
   * LOWER() для case-insensitive поиска (ANSI-совместимо) - Паттерн '%query%' для поиска подстроки
   *
   * <p>Для production: - Используйте SolrSearchService, OpenSearchService или TypeSenseService -
   * Они поддерживают полнотекстовый поиск с индексацией - Гораздо быстрее для больших объемов
   * данных
   *
   ...
   *
   * <p>⚡ ОПТИМИЗАЦИЯ ПРОИЗВОДИТЕЛЬНОСТИ: Порядок условий в SQL влияет на производительность! Более
   * селективные условия (category, status) должны быть первыми, чтобы БД могла использовать индексы
   * и уменьшить количество проверяемых строк перед применением медленного LIKE поиска.
   *
   * <p>Рекомендуемый порядок в fromFilter(): 1. category, status (точные совпадения, используют
   * индексы) ← БЫСТРО 2. author (LIKE, но более селективно чем content) ← СРЕДНЕ 3. query
   * (полнотекстовый поиск, самый медленный) ← МЕДЛЕННО 4. dates (диапазоны, используют индексы) ←
   * БЫСТРО
   *
   * <p>Оптимизация: - Использует Stream API для эффективной фильтрации null значений - Пропускает
   * пустые спецификации (null) - Минимизирует количество операций AND
   *
   ...
   *
   * <p>⚡ ОПТИМИЗИРОВАННЫЙ ПОРЯДОК УСЛОВИЙ: Порядок важен для производительности! Более селективные
   * условия первыми.
   *
   * <p>Порядок: 1. category, status - точные совпадения (используют индексы) ← БЫСТРО 2. dates -
   * диапазоны (используют индексы) ← БЫСТРО 3. author - LIKE поиск (более селективно) ← СРЕДНЕ 4.
   * query - полнотекстовый поиск (самый медленный) ← МЕДЛЕННО
   *
   * <p>Это позволяет БД: - Сначала отфильтровать по индексированным полям - Уменьшить количество
   * строк для проверки LIKE - Использовать индексы максимально эффективно
   */
  public static Specification<Document> fromFilter(
