...
import com.example.search.service.search.SimpleSearchService;
...
public class DocumentResolver {
    ...
    public SearchResultPageDto searchDocuments(@Argument SearchFilterInputDto filter) {
        ...
        log.info("Search request from user {}: query={}, category={}, status={}", 
                userId, 
        ...
        try {
            ...
            log.info("Search completed in {}ms, found {} documents for user {}", 
                    duration, result.getTotalElements(), userId);
    ...
    private SearchFilterDto convertToSearchFilterDto(SearchFilterInputDto input) {
        ...
        SearchFilterDto.SearchFilterDtoBuilder builder = SearchFilterDto.builder()
    ...
    private String convertSortField(String sortField) {
        ...
        switch (sortField.toUpperCase()) {
            case "CREATED_AT": return SortConstants.SORT_BY_CREATED_AT;
            case "UPDATED_AT": return SortConstants.SORT_BY_UPDATED_AT;
            case "TITLE": return SortConstants.SORT_BY_TITLE;
            case "AUTHOR": return SortConstants.SORT_BY_AUTHOR;
            case "CATEGORY": return SortConstants.SORT_BY_CATEGORY;
            case "STATUS": return SortConstants.SORT_BY_STATUS;
            case "RELEVANCE": return SortConstants.SORT_BY_CREATED_AT; // по умолчанию
            default: return SortConstants.SORT_BY_CREATED_AT;
        }
    }
    ...
    public List<AutocompleteResultDto> autocomplete(
        ...
        log.info("Autocomplete request from user {}: prefix={}, type={}, limit={}", 
                userId, prefix, type, limit);
        ...
        try {
            ...
            List<AutocompleteResultDto> results = unifiedSearchService.autocomplete(prefix, type, limit, userId);
    ...
    public Document createDocument(@Argument DocumentInputDto input) {
        ...
        if (!permissionService.canCreateDocument(userId, input.getCategory())) {
            throw new RuntimeException("Access denied: cannot create document in category " + input.getCategory());
        }
