 ...
 *
 * <p>Что делает: - Логирует все GraphQL запросы с временем выполнения - Автоматически обнаруживает
 * медленные запросы (>500ms) - Анализирует частоту запросов для определения hot queries - Собирает
 * метрики для анализа производительности
 *
 * <p>Как использовать: - Все запросы автоматически логируются - Медленные запросы (>500ms)
 * логируются с предупреждением - Каждую минуту выводится статистика hot queries - Запросы-кандидаты
 * на кэширование логируются с префиксом "🔥 CACHE CANDIDATE"
 */
@Configuration
...
public class GraphQLMonitoringConfig {
   ...
   *
   * <p>Этот interceptor перехватывает все GraphQL запросы и: 1. Логирует время выполнения 2.
   * Обнаруживает медленные запросы 3. Анализирует частоту для определения hot queries
   */
  @Bean
  ...
  private void logSlowQuery(long duration, String query, WebGraphQlResponse response) {
    ...
    log.warn(
        "⚠️ SLOW GraphQL query detected: {}ms\nQuery: {}\nErrors: {}",
        duration,
        truncatedQuery,
        errors);
  }
