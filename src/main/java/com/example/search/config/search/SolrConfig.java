 ...
 * - Retry logic для обработки временных сбоев
 * - Правильные таймауты для различных операций
 * - Health check поддержка
 * 
 * HttpSolrClient использует Apache HttpClient под капотом
 * Compatible with Spring Boot 3.2.x и Solr 9.4.x
 * 
 * References:
 * - https://solr.apache.org/guide/solr/9_4/deployment-guide/solrj.html
...
public class SolrConfig {
    ...
    public PoolingHttpClientConnectionManager connectionManager() {
        ...
        log.info("Solr connection pool configured: maxTotal={}, maxPerRoute={}", 
    ...
    public CloseableHttpClient httpClient(PoolingHttpClientConnectionManager connectionManager) {
        ...
        log.info("Solr HttpClient configured: connectionTimeout={}ms, socketTimeout={}ms", 
     ...
     * 
     * Примечание: HttpSolrClient помечен как deprecated в Solr 9.0+, но это единственный
     * вариант для Spring Boot 3.2, так как Http2SolrClient требует Jetty 10,
     * а Spring Boot 3.2 использует Jetty 12 (несовместимо).
     */
    @Bean(destroyMethod = "close")
