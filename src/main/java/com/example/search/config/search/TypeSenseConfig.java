 ...
 * 
 * Основные настройки:
 * - Connection timeout для управления временем ожидания соединения
 * - Поддержка множественных нод для высокой доступности
 * - Retry logic встроен в клиент
 * 
 * References:
...
public class TypeSenseConfig {
     ...
     */
    @Bean
    @ConditionalOnProperty(name = "search.typesense.enabled", havingValue = "true", matchIfMissing = true)
    public Client typeSenseClient() {
        ...
        log.info("TypeSense client initialized: url={}, timeout={}s, retries={}", 
                typesenseUrl, connectionTimeoutSeconds, numRetries);
